  ...
  return (
    <div className=''>
      <header className='bg-[#222222] opacity-90 min-h-[360px] flex justify-center mb-56'>
        ...
        >
          {loggedIn && (
            <button className='absolute top-32 left-32 text-xs text-white hover:text-indigo-600 transition duration-300 ease-in-out '>
              <Edit />
            </button>
          )}
        </div>
            ...
            {profile?.socials?.map((social, index) => {
              return (
                <li key={index} className='relative'>
                  ...
                  {loggedIn && (
                    <button className='absolute top-0 right-0 text-xs text-slate-300 hover:text-indigo-600  transition duration-300 ease-in-out'>
                      <Edit />
                    </button>
                  )}
                </li>
